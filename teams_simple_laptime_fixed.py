#!/usr/bin/env python3
"""
🔧 修正版: Teams共有用シンプル表示版ラップタイム計測システム
- 時間表示の遅延修正
- カメラインデックス問題修正
- 表示更新最適化
"""
...
class TeamsSimpleLaptimeSystemFixed:
    def __init__(self):
        ...
        pygame.display.set_caption("🏁 Lap Timer - Teams View (Fixed)")
    ...
    def draw_camera_feeds(self):
        ...
        # StartLine camera (右側・小さく表示 - 当初設計通り + 検知処理表示)
        ...
        startline_label = self.font_medium.render("StartLine", True, self.colors['text_white'])
    ...
    def run(self):
        """メイン実行ループ（最適化版）"""
        print("📱 Teams共有用シンプル表示版ラップタイム計測システム起動 (修正版)")
...
def main():
    print("🔧 Teams共有用シンプル表示版ラップタイム計測システム (修正版)")
    print("=" * 70)
