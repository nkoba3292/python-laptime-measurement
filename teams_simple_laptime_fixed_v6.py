#!/usr/bin/env python3
"""
Teams共有用シンプル表示タイム計測システム (v6 - 物体検知感度向上版)
- v5より感度を大幅向上：motion_pixels_threshold 600, min_contour_area 500
- 検知条件を3/6以上に緩和してより敏感に反応
- 手の動きや小さな物体でも確実に検知
"""
...
class TeamsSimpleLaptimeSystemFixedV6:
    def __init__(self):
        self.screen_width = 1280
        self.screen_height = 720
        self.screen = pygame.display.set_mode((self.screen_width, self.screen_height))
        pygame.display.set_caption("🏁 Lap Timer - Teams View (v6 - High Sensitivity)")
        self.colors = {
            'background': (15, 15, 25),
    ...
    def load_config(self):
        try:
            ...
            self.config = {
                ...
                "detection_settings": {
                    "motion_pixels_threshold": 600,      # v5:900→600 (大幅感度向上)
                    "min_contour_area": 500,             # v5:1000→500 (小さい動きも検知)
                    "motion_area_ratio_min": 0.015,      # v5:0.025→0.015 (より小さい面積比)
            ...
            print("⚠️ config.json not found, using v6 high sensitivity settings")
    ...
    def evaluate_detection(self, motion_data):
        ...
        if conditions_met >= 3:
            ...
            print(f"🎯 車両検知成功: 条件クリア ({conditions_met}/6) - 高感度検知")
            return True
        return False
    ...
    def draw_camera_feeds(self):
        ...
        startline_label = self.font_medium.render("StartLine (High Sensitivity)", True, self.colors['text_white'])
    ...
    def draw_simple_race_info(self):
        ...
        version_text = "v6 - High Sensitivity"
    ...
    def show_detection_settings(self):
        ...
        print(f"  動きピクセル閾値: {ds['motion_pixels_threshold']} (高感度)")
        print(f"  最小輪郭面積: {ds['min_contour_area']} (高感度)")
        print(f"  面積比範囲: {ds['motion_area_ratio_min']:.3f} - {ds['motion_area_ratio_max']:.3f} (高感度)")
        print(f"  検知条件: 3/6以上 (高感度)")
    ...
    def save_screenshot(self):
        ...
        filename = f"screenshots/teams_view_v6_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        pygame.image.save(self.screen, filename)
        print(f"📸 スクリーンショット保存: {filename}")

    def run(self):
        print("🏁 Teams共有用シンプル表示タイム計測システム起動 (v6 - 物体検知高感度版)")
        print("🎮 操作: R=リセット, Q/ESC=終了, T=タイム表示切替, S=スクリーンショット, C=カメラ情報, D=検知設定")
        print("🔧 v6特徴: 手の動きや小さな物体でも確実に検知する高感度設定")
...
def main():
    print("🏁 Teams共有用シンプル表示タイム計測システム (v6 - 物体検知高感度版)")
    print("=" * 70)
    try:
        system = TeamsSimpleLaptimeSystemFixedV6()
        success = system.run()
        if success:
            print("✅ システム正常終了")
        else:
